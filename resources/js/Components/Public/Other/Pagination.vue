...
const btnColor = ref(isBackend.value ? '#ABABAB' : '#8269AC');
const textColor = ref(isBackend.value ? 'black' : '#8269AC');
const currentTextColor = ref(isBackend.value ? 'black' : 'white');
...
.page {
  @apply md:min-w-[30px] min-w-6 md:leading-7 leading-6 rounded-[50%] text-center text-[v-bind(textColor)] cursor-pointer;
}
...
.current {
  @apply cursor-default text-[v-bind(currentTextColor)] bg-[v-bind(btnColor)];
}
