...
<template>
  <VueDatePicker v-model="dateValue" v-bind="dateOption" ref="datepicker" @cleared="emit('cleared')">
    <template #dp-input="{ value }">
      <label tabindex="0" :title="dateOption?.placeholder ?? '請選擇時間'" :class="twMerge('flex items-center border-black bg-white focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm cursor-pointer', !props.noBorder && 'px-2 border', props.labelClass)" class="rounded-none border " @keyup.enter="openMenu">
        <input type="text" :value="value" :class="twMerge('w-full border-none focus:border-white/0 focus:ring-0 rounded-md py-2 px-3', props.inputClass, !dateOption.disabled && 'cursor-pointer')" :placeholder="dateOption?.placeholder ?? '請選擇時間'" readonly>
...
<style>
.dp__action_buttons .dp__action_button {
  white-space: nowrap;
}
</style>
